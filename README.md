...
본 프로젝트는 안정성과 보안, 확장성을 고려한 **Enterprise-grade 아키텍처**로 설계되었습니다.
...
| 📩 [SQS 비동기 파이프라인](docs/architecture/SQS_FLOW.md) | BE-AI 서버 간의 메시지 워크플로우 |
...
| 🗑️ [데이터 삭제 전략 (Soft/Hard Delete)](docs/architecture/soft-hard-delete-flow.md) | 안정적인 데이터 관리를 위한 삭제 메커니즘 흐름 및 복구(Restore) 전략 가이드 |
| 🛡️ [데이터 정리 메커니즘 (Cleanup)](docs/architecture/cleanup-mechanism.md) | 30일 경과 항목 자동 영구 삭제 및 연쇄 삭제 설계 |
| 🌳 [데이터 생명주기 및 계층 복구](docs/architecture/data-lifecycle.md) | 삭제/복구 시의 Cascade 효과 및 고아 데이터 방지(Move to Root) 로직 |
| 📦 [FE SDK 내부 구조](docs/architecture/fe-sdk-architecture.md) | 프론트엔드 SDK 설계 원리, http-builder, File API 동작 방식 |
| 🔄 [동기화 아키텍처 (LWW)](docs/architecture/sync-lww-logic.md) | 타임스탬프 기반 LWW 동기화 로직 및 정합성 설계 |
| 🔄 [재시도 정책 (Retry)](docs/architecture/retry-policy.md) | 외부 서비스 통신 및 일시적 오류 해결을 위한 재시도 전략 |
| 🧪 [통합 테스트(E2E) Flow](docs/architecture/integrated-testing-flow.md) | BE-AI 연동 검증을 위한 통합 테스트 아키텍처 및 시나리오 |
...
│   ├── tools/            #   - 개별 도구(Note, Conversation 등) 구현부
...
│   ├── errors/           #   - 커스텀 에러 클래스
│   ├── utils/            #   - 헬퍼 함수, 로거
│   └── ai-providers/     #   - 멀티 LLM(OpenAI, Gemini 등) 통합 인터페이스
│
...
더 자세한 폴더의 역할은 **[프로젝트 구조 상세 문서](docs/PROJECT_STRUCTURE.md)**를 참고하세요.
...
```bash
# 설치 (Node.js 환경으로 개발하므로 npm을 권장합니다)
...
- `infisical export` 명령어를 통해 주입된 환경 변수를 확인할 수 있습니다.
...
- **Architecture**:
  ...
  - [웹 기능 명세서 (Functional Specification)](functional_specification.md)
