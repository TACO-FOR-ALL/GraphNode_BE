...
export class MessageService {
   ...
   */
  async updateMessage(ownerUserId: string, conversationId: string, messageId: string, updates: Partial<Omit<ChatMessage, 'id'>> & { updatedAt?: number }): Promise<ChatMessage> {
    try {
   ...
   */
  async updateDoc(messageId: string, conversationId: string, updates: Partial<MessageDoc>, session?: ClientSession): Promise<MessageDoc | null> {
   ...
   * @returns 삭제 성공 여부
   */
  async deleteDoc(messageId: string, conversationId: string, permanent: boolean = false, session?: ClientSession): Promise<boolean> {
    try {
      if (!messageId || messageId.trim().length === 0) {
        throw new ValidationError('Message id is required');
      }
   ...
   * @returns 복구 성공 여부
   */
  async restoreDoc(messageId: string, conversationId: string, session?: ClientSession): Promise<boolean> {
    try {
      if (!messageId || messageId.trim().length === 0) {
        throw new ValidationError('Message id is required');
      }
   ...
   * @returns 삭제 완료를 나타내는 Promise<void>
   */
  async softDeleteAllByConversationId(conversationId: string, session?: ClientSession): Promise<void> {
