...
export class VectorService {
  constructor(
    ...
    private defaultCollection = 'graph_vectors'
  ) {}
  ...
  async searchForUser(
    ...
    try {
      ...
      if (!Array.isArray(queryVector) || queryVector.length === 0)
        throw new ValidationError('queryVector required');
      ...
      // 벡터 검색 수행
      const hits = await this.store.search(this.defaultCollection, queryVector, {
        filter,
        limit: opts?.limit,
      });
      return hits;
