...
export class ConversationService {
  ...
  async listConversations(
    ...
    try {
      const { items: docs, nextCursor } = await this.conversationRepo.listByOwner(
        ownerUserId,
        limit,
        cursor
      );
  ...
  async updateDoc(
    ...
    try {
      ...
      const updatedDoc: ConversationDoc | null = await this.conversationRepo.update(
        id,
        ownerUserId,
        updatePayload,
        session
