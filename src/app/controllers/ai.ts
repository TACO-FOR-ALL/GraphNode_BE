...
export class AiController {
  ...
  async createConversation(req: Request, res: Response) {
    ...
    // 3. 서비스 호출 (대화방 생성)
    const newThread: ChatThread = await this.chatManagementService.createConversation(
      ownerUserId,
      threadId,
      title,
      messages
  ...
  async getConversation(req: Request, res: Response) {
    ...
    // 서비스 호출 (단일 조회)
    const thread: ChatThread = await this.chatManagementService.getConversation(
      conversationId,
      ownerUserId
    );
  ...
  async updateConversation(req: Request, res: Response) {
    ...
    // 서비스 호출 (업데이트)
    const updatedThread: ChatThread = await this.chatManagementService.updateConversation(
      conversationId,
      ownerUserId,
      updates
    );
  ...
  async createMessage(req: Request, res: Response) {
    ...
    // 서비스 호출 (메시지 생성)
    const newMessage: ChatMessage = await this.chatManagementService.createMessage(
      ownerUserId,
      conversationId,
      messageData
    );
  ...
  async updateMessage(req: Request, res: Response) {
    ...
    // 서비스 호출 (메시지 수정)
    const updatedMessage: ChatMessage = await this.chatManagementService.updateMessage(
      ownerUserId,
      conversationId,
      messageId,
      updates
