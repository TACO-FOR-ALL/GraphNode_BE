    ...
    prismaMock.$transaction.mockImplementation(async (cb: any) => {
      const tx = {
        user: {
          ...
          findUnique: (jest.fn() as any).mockResolvedValue({ userInfoId: 'info-existing' }),
          update: jest.fn(),
        },
        userInfo: {
          create: jest.fn(),
    ...
    prismaMock.$transaction.mockImplementation(async (cb: any) => {
      const tx = {
        user: {
          ...
          findUnique: (jest.fn() as any).mockResolvedValue({ userInfoId: null }),
          update: (jest.fn() as any).mockResolvedValue({}),
        },
        userInfo: {
          create: (jest.fn() as any).mockResolvedValue({ id: 'info-new' }),
          update: (jest.fn() as any).mockResolvedValue({}),
        },
      };
      await cb(tx);
