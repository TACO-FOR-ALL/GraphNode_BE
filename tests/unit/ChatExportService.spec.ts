...
describe('ChatExportService', () => {
  ...
  it('creates export job, uploads file, sends email, and returns downloadable status', async () => {
    ...
    expect(email.sendEmailWithAttachment).toHaveBeenCalledWith(
      expect.objectContaining({ to: TEST_EXPORT_RECIPIENT_EMAIL })
    );
