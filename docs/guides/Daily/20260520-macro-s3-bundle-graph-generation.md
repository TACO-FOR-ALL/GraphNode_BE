...
**OpenAI 키가 없을 때 (이 PR 범위만)**

- `npm run e2e:bundle` — 인프라 + BE만 기동, `macro-s3-bundle.spec.ts` 만 실행 (LLM 불필요). 로컬에서 이미 PASS한 것과 동일.
- 전체 `graph-flow` / `microscope` 는 팀 CI(`BE-AI-flow-test`, GitHub Secrets)에 맡기면 됨.
