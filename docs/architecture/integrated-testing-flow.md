...
- **Macro bundle 스펙**: `tests/e2e/specs/macro-s3-bundle.spec.ts` — `graph-generation/{taskId}/` S3 키 검증 (AI 완료 대기 없음)
- **CI 통합 E2E**: `E2E_SCOPE=full` — `tests/e2e/specs/` 전체 (graph-flow metadata·unknown 확장자·Macro API snapshot 포함)
- **빠른 로컬 게이트**: `E2E_SCOPE=bundle` — `macro-s3-bundle.spec.ts` 만 (`npm run e2e:test:bundle`)
