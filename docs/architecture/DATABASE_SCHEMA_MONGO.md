...
| 필드 | 타입 | 설명 |
|---|---|---|
| **id** | `String` (ULID) | 문서 고유 ID (SQS taskId 기반) |
| **s3Key** | `String` | 원본 파일 S3 경로 |
| **fileName** | `String` | 원본 파일명 |
| **status** | `String` | `PENDING`, `PROCESSING`, `COMPLETED`, `FAILED` (전체 상태) |
| **blockStatus** | `String` | block 파이프라인 상태 (듀얼 SQS 모드 전용, Optional) |
| **nonBlockStatus** | `String` | non-block 파이프라인 상태 (듀얼 SQS 모드 전용, Optional) |
...
| **graphPayloadId** | `String` | non-block 처리 성공 시 Payload 문서 ID (Optional) |
| **blockGraphPayloadId** | `String` | block 처리 성공 시 Block Graph Payload 문서 ID (Optional) |
| **ingestMode** | `String` | `from_graphnode` \| `raw_file` (Optional) |
| **blockModeRequested** | `Boolean` | 듀얼 SQS 모드 여부 (Optional) |
| **outputMode** | `String` | `block` \| `non_block` — AI 출력 모드 (Optional) |
| **visualizationS3Key** | `String` | FE 시각화용 대표 S3 키 (Optional) |
| **standardizedS3Key** | `String` | non-block `standardized.json` S3 키 (Optional) |
| **blockGraphS3Key** | `String` | block `block_graph.json` S3 키 (Optional) |
| **imagesS3Prefix** | `String` | block 모드 `images/` S3 prefix (Optional) |
| **error** | `String` | 실패 시 에러 사유 (Optional) |
| **createdAt** | `String` | 등록 일시 |
| **updatedAt** | `String` | 상태 변경 일시 |

> **듀얼 SQS 상태 전이**: `ingestFromNote` 등 호출 시 `{docId}_block` / `{docId}_nonblock` SQS 2건 발행. 
> `blockStatus=COMPLETED` AND `nonBlockStatus=COMPLETED` 시에만 전체 `status=COMPLETED` 전이.

### microscope_graph_payloads 컬렉션

non-block 파이프라인 처리 완료 후 `standardized.json` 그래프 데이터를 분리 저장합니다.

| 필드 | 타입 | 설명 |
|---|---|---|
| **_id** | `String` (ULID) | `DocumentMetaDoc.graphPayloadId`와 동일 |
| **groupId** | `String` | 소속 워크스페이스 ID |
| **taskId** | `String` | `DocumentMetaDoc.id`와 동일 (base docId) |
| **userId** | `String` | 소유자 ID |
| **graphData** | `Object` | 원본 그래프 `{nodes:[], edges:[]}` |
| **createdAt** | `String` | 저장 일시 |

### microscope_block_graph_payloads 컬렉션

block 파이프라인 완료 후 `block_graph.json` 블록 그래프 구조를 분리 저장합니다 (rawText 제외).

| 필드 | 타입 | 설명 |
|---|---|---|
| **_id** | `String` (ULID) | `DocumentMetaDoc.blockGraphPayloadId`와 동일 |
| **groupId** | `String` | 소속 워크스페이스 ID |
| **taskId** | `String` | base docId (`_block` 접미사 제거 후) |
| **userId** | `String` | 소유자 ID |
| **blockGraph** | `Object` | `{blocks, edges, paths, ordering_rationale}` |
| **blockGraph.blocks** | `Array` | 블록 목록 (rawText 미포함) |
| **blockGraph.edges** | `Array` | 블록 간 DAG 엣지 (`type`: `PREREQUISITE_OF` 등) |
| **blockGraph.paths** | `Array<string[]>` | 추천 학습 경로 |
| **blockGraph.ordering_rationale** | `String` | 블록 정렬 근거 (Optional) |
| **createdAt** | `String` | 저장 일시 |

### microscope_block_rawtext_payloads 컬렉션

각 블록의 원문(rawText)을 별도 저장합니다.
rawTexts 총 크기가 10MB 초과 시 저장하지 않으며 FE는 `blockGraphS3Key` 통해 lazy load 합니다.

| 필드 | 타입 | 설명 |
|---|---|---|
| **_id** | `String` (ULID) | 페이로드 고유 ID |
| **groupId** | `String` | 소속 워크스페이스 ID |
| **taskId** | `String` | base docId |
| **userId** | `String` | 소유자 ID |
| **rawTexts** | `Array<{blockId, rawText}>` | 블록별 원문 배열 |
| **createdAt** | `String` | 저장 일시 |
