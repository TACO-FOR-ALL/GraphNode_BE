...
`src/shared/utils/posthog.ts`는 세 가지 방식의 캡처를 지원합니다.

| 함수 | 목적 | 이벤트명 |
| :--- | :--- | :--- |
| `captureEvent` | 비즈니스 이벤트를 수동/명시적으로 전송 | 호출자 지정 |
| `captureApiCall` | HTTP API 감사 이벤트 전송 (미들웨어 전용) | `api_call` |
| `auditProxy` (자동) | 서비스 메서드 호출 수·소요 시간 자동 기록 | `service_method_call` |
...
| 항목 | 결정 | 이유 |
| :--- | :--- | :--- |
| userId 읽기 시점 | `res.on('finish')` 내 `req.userId` | 미들웨어 등록 시점에는 `authJwt` 미실행, requestStore ctx.userId = undefined |
...
| `service_method_call` | `auditProxy` (자동 래핑) | 서비스 메서드 호출 완료/실패 시 (SSE 제외) | `service`, `method`, `duration_ms`, `success`, `correlationId`, `userId`, `ip` |
...
2.  **Filter events**에서 `api_call` 및 `service_method_call` 선택.
3.  특정 유저로 필터링하려면 `Add filter` → `distinct_id` 선택 후 유저 UUID 입력.
...
5.  (팁) Formula를 사용하여 전체 호출 대비 에러율(`A/B * 100`)을 계산할 수도 있습니다.

#### C. 주요 기능 전환 퍼널 (Product Funnel)
1.  **Insights** → **Funnels**.
2.  Step 1: `note_created` (노트 생성).
3.  Step 2: `graph_generation_requested` (그래프 생성 요청).
4.  Step 3: `macro_graph_generated` (그래프 생성 완료).
5.  **결과**: 사용자가 노트를 만든 후 최종적으로 그래프 결과물을 얻기까지 어디서 이탈하는지(Drop-off)를 시각화합니다.

### 4.3. 대시보드(Dashboards) 구성 추천 패널

자주 확인하는 지표들은 하나의 대시보드에 모아두는 것이 좋습니다.
